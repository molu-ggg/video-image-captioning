...
def test_git_inference_single_image(image_path, model_name, prefix):
    param = {}
    image_path_list = sorted(os.listdir(image_path), key=lambda x: int(x.split('.')[0]))
    image_path = [ image_path+"/"+ i for i in image_path_list]
    print(image_path)
...
def iter_caption_to_json(iter_caption, json_file):
    ...
    print(json.dumps(info), json_file)
    print("****************************************")
